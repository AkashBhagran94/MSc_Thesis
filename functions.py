...
def calc_rand_spin_H(matrix, S_init):  # Randomly configures a single spin and calculates H given adjacency matrix

    s = S_init
    r = random.randint(0, matrix.shape[0] - 1)
    s[r] = -s[r]
...
def sample_gibbs_state_new(beta, matrix, number_of_samples, mixing_time, H_init, S_init,return_dict,pnum):
    ...
    while (len(H_samples) <= number_of_samples):
        ...
        H_new, S_new = calc_rand_spin_H(matrix, S_old)
        T = H_old - H_new
...
def calc_z2z1_new(beta2, beta1, number_of_samples, mixing_time,
...
def calc_Zbk_new(betas, number_of_samples, mixing_time,
                 matrix,return_dict,pnum):  # Calculate partition functions up to m, returns numpy array
